...
- 统计页复习洞察：最近 28 天分别统计记得、模糊、忘记，并列出需要优先回顾的薄弱项。
...
- 前台服务持续计时并显示持久通知的番茄钟（25/5/4 轮循环、暂停/继续/跳过、后台/锁屏可恢复、可绑定项目或任务），量化目标、最近 28 天分维度趋势和事件倒计时。
- 固定提醒、每日进度摘要、安静时段、Android 通知/精确提醒权限适配；反馈支持全局/场景覆盖、声音、振动、APK 内置默认提示音、手机系统铃声选择和本地音频导入。
- 固定提醒可编辑时间、星期、安静时段和绑定对象；今日、专注、统计与设置页的信息分组可独立展开/收起，适合小屏快速浏览；宽屏自动切换为带文字的侧边导航。
- 连续打卡休息日设置；休息日不会打断连续记录。
- 加密 AES-GCM 备份、明文备份、导入预览、合并/替换、数据库迁移；替换前自动保留加密安全快照，选中的本地音效随加密备份迁移。
- GitHub Stable Release 更新：自动/手动检查、可恢复下载进度、暂停/续传、SHA-256/包名/版本/签名证书校验，再交给 Android 安装器，不静默安装。

界面采用暖纸张、番茄红与叶绿的学习工作台风格：今日驾驶舱优先展示必做进度，专注页提供一键番茄钟，设置页集中管理提醒、备份和更新；首次启动提供不注入虚假数据的创建引导。今日页支持回看历史日期和受控补记，设置中可以分别切换番茄、提醒、倒计时的声音/振动反馈。应用启动后每 24 小时自动检查一次 GitHub 稳定版，也支持在“设置 → 更新中心”手动检查。

从旧版本升级时，旧的每日进度提醒设置会在首次启动或导出备份前转换为可编辑的统一提醒；不会新增重复提醒。后续提醒只以“设置 → 固定提醒”中的每日总览条目为准。
