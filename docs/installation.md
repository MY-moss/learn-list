...
- v0.3.6 起，历史日期回看、休息日或没有活跃项目时不会弹出重复待办的“漏做处理”提示。
...
- 设置中的反馈开关可分别控制番茄阶段、固定提醒和倒计时的声音与振动：可使用全局默认、仅声音、仅振动、声音+振动或关闭。提示音区域可以试听系统音效，也可以导入不超过 5 MB 的本地音频；文件会复制到应用私有目录，缺失时回退到系统音效。
